...
class GraphicalCoup(RelativeLayout,Coup):
    ...
    def declare_action_to_inspect(self, action):
        self.announcment_label = Label(pos=(200,200),text='Player {} is trying to {}!'.format(action.executor.num,str(action)))
...
class ButtonGenerator(BoxLayout):
    ...
    class ReturningButton(Button):
        def __init__(self, given_object, callback,**kwargs):
            ...
            self.bind(on_press=lambda x:ButtonGenerator.close_choice(return_val=given_object,callback=callback,buttons=self.parent))
